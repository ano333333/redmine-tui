...
pub(crate) fn run() -> ExitCode {
    ...
    if let Err(error) = crate::logging::initialize_logging() {
        eprintln!("{error}");
        return ExitCode::FAILURE;
    }
    trace_dbg!("start");
    ...
    match spawner.block_on(run_app(&mut host, &editor, &spawner, client, dispatcher)) {
        Ok(()) => ExitCode::SUCCESS,
