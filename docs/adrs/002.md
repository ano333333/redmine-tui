...
8. 競合がない property は upload payload に含める。すべての保存対象 property に競合がない場合は payload を確定し、手順 13 に進む。
9. 競合がある property は競合解決 component に表示し、サーバー現在状態を優先するか、ローカル編集後状態を優先するかをユーザーに選択させる。この選択は、表示時点のサーバー状態に対する暫定的な解決結果として扱う。
10. ユーザーが競合解決を中断した場合は保存を中断する。Store 上の issue と `issue_property_diffs` は更新せず、`IssueState` は `Edited` に戻す。
11. ユーザーが競合解決 component で続行を選択した場合は、サーバーから現在の issue を再取得する。再取得中にも `IssueState` は `Uploading` のままとする。
12. 再取得したサーバー現在状態を使い、property 比較と payload 構築をやり直す。競合解決時に表示したサーバー値と再取得値が同じ property にはユーザーの選択を適用する。値が変わった property の選択は無効として最新値で競合を再判定する。未解決の競合がある場合は、最新のサーバー状態で競合解決 component を更新して手順 9 に戻る。競合がない場合のみ、payload を確定する。
13. 確定した payload をサーバーへ送信する。
14. 競合解決後の再取得または upload でエラーが発生した場合は保存を中断し、エラーメッセージを表示する。Store 上の issue と `issue_property_diffs` は更新せず、`IssueState` は `Edited` に戻す。
15. upload に成功した場合は、Redmine の更新レスポンスに含まれる issue を `SyncIssue` action に詰めて dispatch する。
16. `SyncIssue` は Store 上の issue を丸ごと置き換え、対象 issue の `issue_property_diffs` を破棄し、`IssueState` を `Synced` にする。
...
競合解決 component で作業している間にサーバー側の issue がさらに更新される可能性があるため、続行時はその時点の `server` を再取得する。競合解決時に表示した `server` と再取得値が異なる property については前回の選択を無効とし、以下の判定を再実行する。前回の競合解決結果だけを使って upload に進まない。
...
ここで採用しないのは upload 成功後の追加 GET であり、競合解決後、upload 前の再取得とは別である。
...
競合解決後に issue を再取得して比較をループするのは、競合解決 component の表示中もサーバー状態は固定されないためである。
表示時点の値に対する選択をそのまま upload すると、ポップアップ表示後のサーバー更新を失う可能性がある。
再取得した最新値を基準に競合を再判定し、必要な場合はユーザーに再度選択を求めることで、古いサーバー状態に基づく上書きを防ぐ。
...
競合解決 component の続行操作は直接 upload を開始せず、サーバー再取得と property 比較の再実行を要求する。実装は「取得・比較・必要なら競合解決」を繰り返せる制御フローを持つ必要がある。
